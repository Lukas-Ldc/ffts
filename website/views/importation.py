    ...
    if Account.objects.all().filter(user__exact=request.user, unique__exact=account).exists():
        ...
        if request.POST:
            ...
            if "import_ib" in request.POST:
                ib_importer(request.FILES['file'], Account.objects.all().get(user__exact=request.user, unique__exact=account).unique, request.POST['tr_type'], request.POST['ac_type'])
